          ...
          <div className="space-y-4">
            ...
            {aligningStatus?.status === 'running' ? (
              ...
              <div className="bg-[#15151a] border border-gray-800 rounded-xl p-4 space-y-4">
                ...
                {/* BLOCK 3.5: 🎙️ ACOUSTIC ANALYSIS & AUTO-SEGMENTATION SUITE */}
                <div className="bg-gradient-to-br from-[#101920] to-[#12161f] border border-emerald-500/35 rounded-xl p-3.5 space-y-3 shadow-lg">
                  <div className="flex items-center justify-between pb-1 border-b border-emerald-500/20">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-xs font-extrabold text-emerald-300 uppercase tracking-wide">
                        ACOUSTIC ANALYSIS & AUTO-SEGMENTATION (آٹو سیگمنٹیشن)
                      </span>
                    </div>
                    <span className="text-[9px] font-mono px-2 py-0.5 rounded bg-emerald-500/10 text-emerald-300 font-bold border border-emerald-500/25">
                      Adaptive VAD
                    </span>
                  </div>

                  <p className="text-[10px] text-gray-300 leading-relaxed">
                    Automatically analyze recitation voice energy, acoustic breathing pauses, and scene transitions to slice audio and synchronize background videos.
                  </p>

                  {/* Sensitivity Presets */}
                  <div className="space-y-1.5">
                    <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block">
                      Acoustic Preset / Recitation Style:
                    </span>
                    <div className="grid grid-cols-2 gap-1.5">
                      {[
                        { id: 'studio', label: '🎙️ Studio Clarity', desc: '180ms silence, -40dB' },
                        { id: 'mosque', label: '🕌 Mosque / Reverb', desc: '320ms silence, -32dB' },
                        { id: 'tartil', label: '📖 Tartil (Slow Madd)', desc: '380ms silence, extended' },
                        { id: 'hadr', label: '⚡ Fast Hadr', desc: '150ms silence, rapid' },
                      ].map((preset) => (
                        <button
                          key={preset.id}
                          type="button"
                          ...
                          className={`p-2 text-left rounded-lg transition border cursor-pointer ${
                            audioSensitivity === preset.id
                              ? 'bg-emerald-500/20 text-emerald-300 border-emerald-400 shadow-md font-bold'
                              : 'bg-[#151a22] text-gray-300 hover:text-white hover:bg-gray-800 border-gray-800'
                          }`}
                        >
                          <div className="text-[11px] font-semibold">{preset.label}</div>
                          <div className="text-[9px] text-gray-400">{preset.desc}</div>
                        </button>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-1">
                    <button
                      type="button"
                      id="btn-media-autosegment-audio"
                      onClick={async () => {
                      ...
                      className="py-2.5 px-3 bg-emerald-600/25 hover:bg-emerald-600/40 border border-emerald-500/50 hover:border-emerald-400 text-emerald-200 font-bold rounded-lg text-xs transition flex items-center justify-center gap-1.5 cursor-pointer shadow-sm disabled:opacity-50"
                    >
                      {isSegmentingAudio ? (
                        <>
                        ...
                        <>
                          ...
                          <span>🎙️ Auto-Segment Audio</span>
                        </>
                      )}
                    </button>
                    ...
                    <button
                      type="button"
                      id="btn-media-autosync-video"
                      onClick={() => {
                        if (onAutoSyncVideoToAyahs) {
                          setIsSyncingVideo(true);
                          onAutoSyncVideoToAyahs();
                          setTimeout(() => setIsSyncingVideo(false), 500);
                        }
                      }}
                      disabled={isSyncingVideo}
                      className="py-2.5 px-3 bg-cyan-600/25 hover:bg-cyan-600/40 border border-cyan-500/50 hover:border-cyan-400 text-cyan-200 font-bold rounded-lg text-xs transition flex items-center justify-center gap-1.5 cursor-pointer shadow-sm disabled:opacity-50"
                    >
                      {isSyncingVideo ? (
                        <>
                        ...
                        <>
                          ...
                          <span>🎬 Auto-Sync Video to Ayahs</span>
                        </>
                      )}
                    </button>
                  </div>

                  {/* Silence Remover Quick Action */}
                  <button
                    type="button"
                    id="btn-media-remove-silence"
                    onClick={() => {
                      if (onAutoRemoveSilence) onAutoRemoveSilence(selectedClip?.id);
                    }}
                    className="w-full py-2 px-3 bg-amber-500/15 hover:bg-amber-500/25 border border-amber-500/30 hover:border-amber-400 text-amber-300 font-semibold rounded-lg text-[11px] transition flex items-center justify-center gap-1.5 cursor-pointer"
                  >
                    <Scissors className="w-3.5 h-3.5 text-amber-400" />
                    <span>✂️ Auto-Trim Dead Silence Gaps (&gt;250ms) &amp; Ripple Align</span>
                  </button>
                </div>
